...
impl GlobalDispatch<ZwlrDataControlManagerV1, ()> for Composer {
    fn bind(
        _state: &mut Self,
...
impl Dispatch<ZwlrDataControlManagerV1, ()> for Composer {
    fn request(
        state: &mut Self,
        ...
        match request {
            ...
            zwlr_data_control_manager_v1::Request::GetDataDevice { id, seat: _ } => {
                ...
                if let Some(source) = &state.selection {
                    let offer = client
                        .create_resource::<ZwlrDataControlOfferV1, (), Self>(
                            dhandle,
                            device.version(),
                            (),
                        )
...
impl Dispatch<ZwlrDataControlDeviceV1, ()> for Composer {
    fn request(
        state: &mut Self,
        _client: &wayland_server::Client,
...
impl Dispatch<ZwlrDataControlSourceV1, ()> for Composer {
    fn request(
        state: &mut Self,
        _client: &wayland_server::Client,
...
impl Dispatch<ZwlrDataControlOfferV1, ()> for Composer {
    fn request(
        state: &mut Self,
        _client: &wayland_server::Client,
